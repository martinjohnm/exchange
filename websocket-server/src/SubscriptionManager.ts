...
export class SubscriptionManager {
    ...
    public unsubscribe(userId: string, subscription: string) {
        ...
        if (existingSubscriptions) {
            this.subscriptions.set(userId, existingSubscriptions.filter(s => s!== subscription));
        }
        const reversExistingSubscriptions = this.reverseSubscriptions.get(subscription)
        if (reversExistingSubscriptions) {
            this.reverseSubscriptions.set(subscription, reversExistingSubscriptions.filter(s => s!== userId))
